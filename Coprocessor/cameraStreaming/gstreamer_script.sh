...
#do
	if [ -e ${CAMERA_FRONT} ]; then
	       ...
	       if [ $? -eq 0 ]; then
		       #gst-launch-1.0 -v v4l2src device=${CAMERA_FRONT} ! "video/x-raw,width=640,height=480,framerate=30/1" ! avenc_mpeg4 ! rtpmp4vpay config-interval=3 ! tcpserversink host=${SELF_IP_ADDR} port=${PORT_FOR_FRONT} & #udpsink host=${DS_IP_ADDR} port=${PORT_FOR_FRONT} &
