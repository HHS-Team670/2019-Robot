...
public class PlaceOrGrab extends CommandGroup {
   ...
   */
  public PlaceOrGrab(LegalState state, boolean isPlacing) {
    if (state.equals(LegalState.PLACE_BALL_CARGOSHIP_BACK)) {
      addSequential(new DropBall(Robot.claw));
    } else if (state.equals(LegalState.PLACE_BALL_CARGOSHIP_FORWARD)) {
      addSequential(new DropBall(Robot.claw));
    } else if (state.equals(LegalState.READY_PLACE_BALL_ROCKET_LOW_BACK)) {
      addSequential(new MoveArm(Arm.getArmState(LegalState.PLACE_BALL_ROCKET_LOW_BACK), Robot.arm));
      addSequential(new DropBall(Robot.claw));
      addSequential(new MoveArm(Arm.getArmState(state), Robot.arm));
    } else if (state.equals(LegalState.READY_PLACE_BALL_ROCKET_LOW_FORWARD)) {
      addSequential(new MoveArm(Arm.getArmState(LegalState.PLACE_BALL_ROCKET_LOW_FORWARD), Robot.arm));
      addSequential(new DropBall(Robot.claw));
      addSequential(new MoveArm(Arm.getArmState(state), Robot.arm));
    } else if (state.equals(LegalState.READY_PLACE_BALL_ROCKET_MIDDLE_BACK)) {
      addSequential(new MoveArm(Arm.getArmState(LegalState.PLACE_BALL_ROCKET_MIDDLE_BACK), Robot.arm));
      addSequential(new DropBall(Robot.claw));
      addSequential(new MoveArm(Arm.getArmState(state), Robot.arm));
    } else if (state.equals(LegalState.READY_PLACE_BALL_ROCKET_MIDDLE_FORWARD)) {
      addSequential(new MoveArm(Arm.getArmState(LegalState.PLACE_BALL_ROCKET_MIDDLE_FORWARD), Robot.arm));
      addSequential(new DropBall(Robot.claw));
      addSequential(new MoveArm(Arm.getArmState(state), Robot.arm));
    } else if (state.equals(LegalState.READY_PLACE_HATCH_ROCKET_MIDDLE_BACK)) {
      ...
      addSequential(new MoveArm(Arm.getArmState(state), Robot.arm));
    } else if (state.equals(LegalState.READY_PLACE_HATCH_ROCKET_MIDDLE_FORWARD)) {
      addSequential(new MoveArm(Arm.getArmState(LegalState.PLACE_HATCH_ROCKET_MIDDLE_FORWARD), Robot.arm));
      addSequential(new DropHatch(Robot.claw));
      addSequential(new MoveArm(Arm.getArmState(state), Robot.arm));
    } else if (state.equals(LegalState.GRAB_BALL_GROUND_BACK)) {
      addSequential(new PickupBall(Robot.claw));
    } else if (state.equals(LegalState.GRAB_BALL_INTAKE)) { // check this
      addSequential(new AutoPickupCargo(Robot.arm, Robot.intake, Robot.claw, Robot.sensors));
    } else if (state.equals(LegalState.READY_GRAB_BALL_LOADINGSTATION_BACK)) { // figure this out
      addSequential(new MoveArm(Arm.getArmState(LegalState.GRAB_BALL_LOADINGSTATION_BACK), Robot.arm));
      addSequential(new PickupBall(Robot.claw));
      addSequential(new MoveArm(Arm.getArmState(state), Robot.arm));
    } else if (state.equals(LegalState.READY_GRAB_BALL_LOADINGSTATION_FORWARD)) { // figure this out
      addSequential(new MoveArm(Arm.getArmState(LegalState.GRAB_BALL_LOADINGSTATION_FORWARD), Robot.arm));
      addSequential(new PickupBall(Robot.claw));
      addSequential(new MoveArm(Arm.getArmState(state), Robot.arm));
    } else if (state.equals(LegalState.READY_LOW_HATCH_BACK)) {
      if (isPlacing) {
    ...
    } else if (state.equals(LegalState.READY_LOW_HATCH_FORWARD)) {
      if (isPlacing) {
